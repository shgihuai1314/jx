layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            if (_config.maxSetting !== 'undefined') {
                ...
                if (typeof _config.maxSetting === 'number') {
                    if (currentTabCount === _config.maxSetting) {
                        layer.msg('为了系统的流畅度，只能同时打开' + _config.maxSetting + '个选项卡。', {offset: '150px'});
                        return;
                    }
                }
                if (typeof _config.maxSetting === 'object') {
                    ...
                    if (currentTabCount === max) {
                        layer.msg(msg, {offset: '150px'});
                        return;
                    }
                }
            }
            globalTabIdIndex++;
            var content = '<iframe frameborder="0" src="' + data.href + '" data-id="' + globalTabIdIndex + '"></iframe>';
            var title = '';
            ...
            element.tabChange(ELEM.tabFilter, that.getTabId(data.title));
            //自动刷新
            if (data.refresh || _config.autoRefresh) {
                //console.log('刷新了');
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                case 'refresh':
                                    ...
                                    // 刷新iframe中当前显示的url
                                    if (ELEM.contentBox.find('iframe[data-id=' + id + ']')[0]) {
